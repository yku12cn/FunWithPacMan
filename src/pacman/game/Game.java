...
public final class Game
{
	...
	//private int NUM_GHOSTS = 3;		// Default number of ghosts
...
/*	public Game(long seed)
	...
	public Game(long seed, int initialMaze, int speed_reduction, int nGhosts){
		
		constants.NUM_GHOSTS = nGhosts;
		constants.GHOST_SPEED_REDUCTION = speed_reduction;
		this.seed=seed;
		rnd=new Random(seed);
	...
	public Game(long seed,int initialMaze, int speed_reduction)
	{						
		constants.GHOST_SPEED_REDUCTION = speed_reduction;
		this.seed=seed;
		rnd=new Random(seed);
...
/*
	...
	{			
		this(seed, initialMaze, new Constants());	
	}
	
	public Game(long seed, int initialMaze, Constants constants){
		this.seed = seed;
		rnd = new Random(seed);
...
*/	
	...
	public Game(Random rGen, Constants constants){
		rnd = rGen;
		this.constants = constants;
		
		_init(constants.MAZE_NUM);
	}
	
	public Game (long seed, Constants constants){
		this.seed = seed;
		rnd = new Random(seed);
		this.constants = constants;
	...
	private void _init(int initialMaze)
	{
		...
		pacman=new PacMan(currentMaze.initialPacManNodeIndex,MOVE.LEFT,constants.NUM_LIVES,false);		
	}
	...
	private void _initGhosts()
	{
		...
		for(GHOST ghostType : GHOST.values()){
			int lairTime = (int)(ghostType.initialLairTime*(Math.pow(constants.LAIR_REDUCTION,levelCount)));			
			if (++ghostCount > constants.NUM_GHOSTS)
				lairTime = Integer.MAX_VALUE;			
			ghosts.put(ghostType,new Ghost(currentMaze.lairNodeIndex, 0, lairTime ,MOVE.NEUTRAL));
		}
	}
	...
	{
		if(!pacman.hasReceivedExtraLife && score>=constants.EXTRA_LIFE_SCORE)	//award 1 extra life at 10000 points
		{
	...
	private void _updateGhosts(EnumMap<GHOST,MOVE> moves)
	{
		...
		if(moves.size()<constants.NUM_GHOSTS)
			for(GHOST ghostType : GHOST.values())
		...
		for(Entry<GHOST,MOVE> entry : moves.entrySet())
		{
			...
			if(ghost.lairTime==0)
			{
				...
				else if(ghost.edibleTime==0 || ghost.edibleTime%constants.GHOST_SPEED_REDUCTION!=0)
				{
	...
	private void _eatPowerPill()
	{
		...
		{
			score+=constants.POWER_PILL;
			ghostEatMultiplier=1;
			powerPills.clear(powerPillIndex);
			
			int newEdibleTime=(int)(constants.EDIBLE_TIME*(Math.pow(constants.EDIBLE_TIME_REDUCTION,levelCount)));
		...
		else if(currentLevelTime>1 && Math.random()<constants.GHOST_REVERSAL && enableGlobalReversals)	//random ghost reversal
		{
	...
	{		
		...
		for(Ghost ghost : ghosts.values())
		{
			...
			if(distance<=constants.EAT_DISTANCE && distance!=-1)
			{
				...
				{
					score+=constants.GHOST_EAT_SCORE*ghostEatMultiplier;
					//ghostEatMultiplier*=2;
					ghost.edibleTime=0;					
					ghost.lairTime=(int)(constants.COMMON_LAIR_TIME*(Math.pow(constants.LAIR_REDUCTION,levelCount)));					
	...
	{				
		...
		if((pills.isEmpty() && powerPills.isEmpty()) || currentLevelTime>=constants.LEVEL_LIMIT)
		{
			//award any remaining pills to Ms Pac-Man
			score+=(int)(constants.PILL_REWARD_REDUCTION*(constants.PILL*pills.cardinality()+constants.POWER_PILL*powerPills.cardinality()));			 
			
			// if all pills were eaten, award bonus
			if (pills.isEmpty() && powerPills.isEmpty()){
				score += constants.BOARD_COMPLETION_BONUS;
			}
			
			//put a cap on the total number of levels played
			if(levelCount+1==constants.MAX_LEVELS)
				gameOver=true;
			else
				_newLevelReset();
		}
	...
	/**
	 * Get an ArrayList with all ghosts
	 * 
	...
	{
		return constants.GHOST_EAT_SCORE*ghostEatMultiplier;
	}
	...
	public boolean doesGhostRequireAction(GHOST ghostType)
	{
		...
		return ((isJunction(ghosts.get(ghostType).currentNodeIndex) || (ghosts.get(ghostType).lastMoveMade==MOVE.NEUTRAL) && ghosts.get(ghostType).currentNodeIndex==currentMaze.initialGhostNodeIndex) 
				&& (ghosts.get(ghostType).edibleTime==0 || ghosts.get(ghostType).edibleTime%constants.GHOST_SPEED_REDUCTION!=0));	
	}
	...
	public boolean isJunction(int nodeIndex)
	{
		...
		//System.out.println(currentMaze.graph[nodeIndex]);
		//if (nodeIndex == -1)
		//	return false;
