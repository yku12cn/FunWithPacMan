...
public class PathsCache 
{
 	...
 	public PathsCache(int mazeIndex)
	{
		...
		constants.MAZE_NUM = mazeIndex;
		
		this.game=new Game(0,constants);
		Maze m = game.getCurrentMaze();
		
		// An array of all the junction node indices
		int[] jctIndices = m.junctionIndices;
/*		
		if (mazeIndex == 4){
			System.out.println("Junctions:");
			for (Integer i: jctIndices){
				System.out.print(i + " ");
			}
			System.out.println();
		}
*/		
		// A mapping from node index to junction index
		...
		//System.out.printf("INITIATED PATH CACHE %d\n", mazeIndex);
	}
	...
	private DNode[] assignJunctionsToNodes(Game game)
	{
		...
		for (int i = 0; i < numNodes; i++) 
		{
			...
			if(!isJunction)
			{
				...
				// Get all possible moves at that node
...
/*	
				...
				System.out.println();
*/				
				// For each direction starting from that node
				for (int j = 0; j < possibleMoves.length; j++)
				{
					...
					//System.out.println("Moving " + lastMove + ". Now at " + currentNode );
					...
					while (!game.isJunction(currentNode)) 
					{
						...
						//System.out.println("Now at " + currentNode);
